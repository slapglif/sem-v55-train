...
class HybridAutomata(nn.Module):
    ...
    def __init__(
        ...
        if learnable_threshold:
            ...
            self.register_buffer(
                "log_threshold", torch.tensor(math.log(curvature_threshold))
            )
    ...
    def compute_curvature(self, H_t: Tensor, H_prev: Tensor, dt: float = 1.0) -> Tensor:
...
def checkpoint_hybrid_automata(
    automata: HybridAutomata, psi: Tensor, H_t: Tensor, H_prev: Tensor, dt: float = 1.0
) -> Tuple[Tensor, Tensor]:
...
def attention_to_hamiltonian(
    attn_weights: Tensor, make_hermitian: bool = True
) -> Tensor:
    ...
    if make_hermitian:
        ...
        H = safe_complex(attn_weights, torch.zeros_like(attn_weights))
